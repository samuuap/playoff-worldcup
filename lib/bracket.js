...
// ENTRADA TARDÍA — "bloqueo por partido".
// Si un usuario fue habilitado tarde (lateAt), no puntúa SOLO las ranuras cuyo
// resultado ya se conocía cuando entró. Los partidos aún por jugar SÍ puntúan,
// aunque sean de la rama de un equipo que ya había ganado antes: predecir esos
// (p. ej. que Brasil pierde su siguiente) es incierto para todos por igual.
export function taintedSet(matches, order, lateAt){
  ...
  for (const id of order) {
    const m = matches[id];
    if (m && m.result_set_at && new Date(m.result_set_at).getTime() < t) tainted.add(id);
  }
  return tainted;
}
