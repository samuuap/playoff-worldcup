...
// Equipos que YA se sabían vivos (con una victoria registrada) cuando entró el tardío.
export function knownAliveTeams(matches, order, lateAt){
  const alive = new Set();
  if (!lateAt) return alive;
  const t = new Date(lateAt).getTime();
  for (const id of order) {
    const m = matches[id];
    if (m && m.actual_winner && m.result_set_at && new Date(m.result_set_at).getTime() < t)
      alive.add(m.actual_winner);
  }
  return alive;
}

// ENTRADA TARDÍA — "no se cobra por rodar al ganador ya conocido".
// Si el usuario entró tarde, NO puntúa una ranura donde apostó a que GANA un equipo
// que ya se sabía vivo cuando entró (era información gratis). En cambio, apostar a que
// ese equipo PIERDE (eligiendo a su rival) sí puntúa: eso es mérito propio.
// Devuelve el Set de ids de ranura cuyo pick no debe puntuar.
export function taintedSet(matches, order, lateAt, picks = {}){
  if (!lateAt) return new Set();
  const alive = knownAliveTeams(matches, order, lateAt);
  const tainted = new Set();
  for (const id of order) {
    if (picks[id] && alive.has(picks[id])) tainted.add(id);
  }
  return tainted;
}
